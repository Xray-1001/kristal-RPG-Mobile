...
function makeMonster(floorId, isBoss, forcedSpriteKey){
...
// Koordinat mengikuti tata letak gambar lobby (kanvas 910x607, skala 1.3x dari sebelumnya)
const LOBBY_BUILDINGS = [
  {name:"Balai Guild",           x:442, y:124, r:57, action:"guild"},
  {name:"Toko Senjata & Armor",  x:237, y:140, r:57, action:"shop"},
  {name:"Merchant Ramuan",       x:628, y:140, r:57, action:"merchant"},
  {name:"Gerbang Dungeon",       x:794, y:164, r:60, action:"dungeon_select"},
  {name:"Penginapan",            x:127, y:289, r:55, action:"inn"},
  {name:"Bank",                  x:586, y:299, r:52, action:"bank"},
  {name:"Tukang Besi",           x:738, y:299, r:52, action:"blacksmith"},
  {name:"Papan Misi",            x:285, y:471, r:49, action:"questboard"},
  {name:"Pasar",                 x:605, y:471, r:52, action:"market"},
];
// Tembok tak terlihat (penghalang) - perkiraan posisi bangunan & sungai berdasarkan gambar lobby.
// Kalau ada yang meleset (kelewat longgar/ketat), kasih tahu bagian mananya biar bisa disesuaikan lagi.
const LOBBY_SOLIDS = [
  {x:387, y:39,  w:110, h:75}, // Balai Guild
  {x:182, y:55,  w:110, h:75}, // Toko
  {x:573, y:55,  w:110, h:75}, // Merchant
  {x:739, y:79,  w:110, h:75}, // Gerbang Dungeon (bingkai batu)
  {x:79,  y:224, w:96,  h:55}, // Penginapan
  {x:538, y:234, w:96,  h:55}, // Bank
  {x:690, y:234, w:96,  h:55}, // Tukang Besi
  {x:237, y:406, w:96,  h:55}, // Papan Misi
  {x:557, y:406, w:96,  h:55}, // Pasar
  // Sungai/air di tepi bawah peta
  {x:0,   y:520, w:280, h:90},
  {x:650, y:520, w:260, h:90},
  {x:280, y:560, w:110, h:50},
  {x:520, y:560, w:110, h:50},
];
...
function drawOverworld(canvas){
  ...
  if(merchantB) drawMonsterSprite(ctx, "wierd_traveler", merchantB.x+65, merchantB.y+59, 32, true);
  drawPlayerSprite(ctx, player.pos.x, player.pos.y, player.facing, player.isMoving);

  LOBBY_BUILDINGS.forEach(b=>{
    const dx=player.pos.x-b.x, dy=player.pos.y-b.y;
...
function removeCurrentMonsterSprite(monster){
  const idx = dungeonMonsters.findIndex(m=>m.alive && (m.isBoss===monster.isBoss));
...
function drawDungeon(canvas, d){
  ...
  dungeonMonsters.filter(m=>m.alive).forEach(m=>{
    ...
    if(Math.sqrt(dx*dx+dy*dy) < 30){
      const monster = makeMonster(player.currentFloor, m.isBoss, m.spriteKey);
      startBattle(monster);
    }
  });
}
