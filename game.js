...
/* ============================== SPRITE PLAYER ============================== */
// Spritesheet berisi 6 frame @84x128, urutan: down_a, down_b, up_a, up_b, side_a, side_b
...
function drawPlayerSprite(ctx, x, y, direction, moving){
  if(!playerSheetLoaded){
    ctx.font="26px serif"; ctx.textAlign="center"; ctx.fillText("🧑", x, y);
    return;
  }
  ...
  else { frames = PLAYER_FRAMES.side; mirror = (direction==="right"); }
  const idx = frames[toggle?1:0];
  const sx = idx*PLAYER_CELL_W;
  const drawH = 36, drawW = drawH*(PLAYER_CELL_W/PLAYER_CELL_H); // disamakan skala dgn NPC
  const bob = moving ? Math.sin(Date.now()/110)*2 : 0; // efek "jalan" krn sprite sumber tak punya frame kaki melangkah
  ctx.save();
  if(mirror){ ctx.translate(x,0); ctx.scale(-1,1); ctx.translate(-x,0); }
  ctx.drawImage(playerSheet, sx, 0, PLAYER_CELL_W, PLAYER_CELL_H, x-drawW/2, y-drawH+10-bob, drawW, drawH);
  ctx.restore();
}
function updateFacingFromKeys(keys){
  // pakai logika eksklusif + pertahankan arah terakhir kalau ada bentrok sinyal tombol (anti kedip)
  if(keys.left && !keys.right) player.facing="left";
  else if(keys.right && !keys.left) player.facing="right";
  else if(keys.up && !keys.down) player.facing="up";
  else if(keys.down && !keys.up) player.facing="down";
...
// Kumpulan monster per lantai dungeon (elemen terakhir di tiap array = sprite utk BOSS lantai itu)
...
function pickMonsterSprite(floorId, isBoss){
  const pool = DUNGEON_MONSTER_POOL[floorId] || ["green_blob"];
...
const WEAPONS = genWeapons();   // 7 x 11 = 77
const ARMORS  = genArmors();    // 5 x 11 = 55
const AMULETS = genAmulets();   // 10 x 5 = 50
...
const SKILL_LEVELS = [
  {name:"Kecil",   mult:1.4, cd:1},
  {name:"Sedang",  mult:2.0, cd:2},
  {name:"Besar",   mult:2.8, cd:2},
  {name:"Dahsyat", mult:3.8, cd:3},
  {name:"Ultimate",mult:5.2, cd:4},
];
const DODGE_SKILLS = [
  "Dodge Reflek I","Dodge Reflek II","Dodge Reflek III","Langkah Bayangan",
  "Mata Elang","Gerakan Kilat","Insting Bertahan","Tarian Menghindar",
  "Auto Dodge","Kewaspadaan Total"
];
const UNIQUE_SKILLS = [
...
function genSkills(){
  ...
  SKILL_ELEMENTS.forEach(el=>{
    SKILL_LEVELS.forEach(lv=>{
      out.push({
        id:`sk_${el}_${lv.name}`,
        type:"attack",
        element: el,
        name:`${lv.name==="Ultimate"?"":lv.name+" "}Serangan ${el}${lv.name==="Ultimate"?" Ultimate":""}`,
        mult: lv.mult,
        cd: lv.cd,
        source: "kill",
  ...
  WEAPON_TYPES.forEach(w=>{
    ["Mahir","Ahli","Master"].forEach((rank,i)=>{
      out.push({
        ...
        source:"weaponkills",
        weapon: w.name,
        needKills:[20,50,100][i],
        desc:`Bonus serangan tetap saat memakai ${w.name}.`
      });
    });
  });
  DODGE_SKILLS.forEach((name,i)=>{
    out.push({
      id:`sk_dodge_${i}`,
      type: name==="Auto Dodge" ? "active_dodge" : "passive_dodge",
      name,
      source:"dodge_streak",
      needStreak: (i+1)*4,
      desc: name==="Auto Dodge" ? "Otomatis menghindar 3x serangan, lalu cooldown." : "Meningkatkan kemampuan menghindar."
    });
  });
...
const SKILLS = genSkills(); // 12*5 + 7*3 + 10 + 18 = 60+21+10+18 = 109

/* ============================== DATA: DUNGEON ============================== */

const DUNGEONS = [
  { id:1, name:"Hutan Pertama",          monsterCount:5,  element:"Bumi",   boss:"Raja Serigala Tunggul", monsterName:"Serigala Hutan" },
  { id:2, name:"Hutan Rawa",             monsterCount:10, element:"Racun",  boss:"Ratu Lintah Rawa",      monsterName:"Siluman Rawa" },
  { id:3, name:"Hutan Kematian",         monsterCount:15, element:"Gelap",  boss:"Penjaga Hutan Terkutuk",monsterName:"Mayat Berjalan" },
  { id:4, name:"Gua Kristal",            monsterCount:20, element:"Bumi",   boss:"Golem Kristal Purba",   monsterName:"Kelelawar Kristal" },
  { id:5, name:"Reruntuhan Kuno",        monsterCount:25, element:"Waktu",  boss:"Penjaga Reruntuhan",    monsterName:"Patung Hidup" },
  { id:6, name:"Lembah Beracun",         monsterCount:30, element:"Racun",  boss:"Naga Rawa Beracun",     monsterName:"Kalajengking Raksasa" },
  { id:7, name:"Benteng Terbengkalai",   monsterCount:35, element:"Baja",   boss:"Kesatria Baja Terkutuk",monsterName:"Prajurit Bayangan" },
  { id:8, name:"Puncak Es Abadi",        monsterCount:40, element:"Es",     boss:"Naga Es Abadi",         monsterName:"Serigala Es" },
  { id:9, name:"Jurang Neraka",          monsterCount:45, element:"Api",    boss:"Iblis Penjaga Jurang",  monsterName:"Iblis Kecil" },
  { id:10,name:"Singgasana Sang Penguasa Kegelapan", monsterCount:0, element:"Gelap", boss:"Penguasa Kegelapan", monsterName:null, finalBoss:true },
];
...
/* ============================== CURRENCY HELPERS ============================== */
// simpan semua uang sebagai total "perunggu" lalu konversi utk tampilan
...
let player = null; // state pemain aktif
...
function makeNewPlayer(username, email){
  return {
    ...
    floorProgress:{}, // {floorId: monstersKilled}
    pos:{x:450,y:250},
    facing:"down",
    isMoving:false,
    autoDodgeCharges:0,
    autoDodgeCooldown:0,
    upgradeLevels:{}, // {itemId: level} - hasil upgrade Tukang Besi
  };
}
...
function playerTotalAtk(){
  ...
  if(w) atk += w.atk + getUpgradeLevel(w.id)*4;
...
function playerTotalDef(){
  ...
  if(a) def += a.def + getUpgradeLevel(a.id)*3;
  const am = getAmulet(player.equipped.amulet);
  if(am) def += am.def;
  return def;
}
...
function makeMonster(floorId, isBoss, forcedSpriteKey, sourceUid){
  const d = DUNGEONS.find(x=>x.id===floorId);
  const f = floorId;
  const baseHp = 25 * Math.pow(f, 1.32);
  const baseAtk = 4 * Math.pow(f, 1.18);
  const mult = isBoss ? 6 : 1;
  return {
    name: isBoss ? d.boss : d.monsterName,
    spriteKey: forcedSpriteKey || pickMonsterSprite(floorId, isBoss),
    isBoss,
    floor: f,
    element: d.element,
    ...
    dodgeDifficulty: Math.min(0.85, 0.15 + f*0.06 + (isBoss?0.12:0)),
    bulletCount: Math.min(9, 2 + Math.floor(f/1.5) + (isBoss?2:0)),
    bulletSpeed: Math.min(6.5, 1.6 + f*0.35 + (isBoss?0.8:0)),
    dialogueIndex: 0,
    sourceUid, // penanda sprite mana persis di peta dungeon yang memicu battle ini
  };
}
...
function onMonsterKilled(monster){
  const learned = [];
  // 1) elemental attack skill dari elemen monster (peluang 22%)
  if(Math.random() < 0.22){
    const candidates = SKILLS.filter(s=>s.source==="kill" && s.element===monster.element && !player.skills.includes(s.id));
    if(candidates.length){
      const pick = candidates[Math.floor(Math.random()*candidates.length)];
      learned.push(tryLearnSkill(pick.id));
    }
  }
  // 2) weapon mastery
  const w = getWeapon(player.equipped.weapon);
  if(w){
    const wtype = WEAPON_TYPES.find(t=> w.name.endsWith(t.name));
    if(wtype){
      player.weaponKillCount[wtype.name] = (player.weaponKillCount[wtype.name]||0)+1;
      const cnt = player.weaponKillCount[wtype.name];
      SKILLS.filter(s=>s.type==="passive" && s.weapon===wtype.name && s.needKills<=cnt && !player.skills.includes(s.id))
        .forEach(s=> learned.push(tryLearnSkill(s.id)));
    }
  }
  // 3) boss unique skill
  if(monster.isBoss){
  ...
  return learned.filter(Boolean);
}

function onDodgeSuccessStreak(){
  player.dodgeStreak++;
  const learned = [];
  SKILLS.filter(s=>s.source==="dodge_streak" && s.needStreak<=player.dodgeStreak && !player.skills.includes(s.id))
    .forEach(s=> { const l = tryLearnSkill(s.id); if(l) learned.push(l); });
  return learned;
}
...
function rollCrystalDrop(monster){
  // makin tinggi lantai / boss, makin besar peluang kristal langka
...
let battle = null; // {monster, log, turn, buffs, dodgeGame:{...}}
...
function currentPlayerAtk(){
  return playerTotalAtk() + battle.playerBuffs.atk;
}
function currentPlayerDef(){
  return playerTotalDef() + battle.playerBuffs.def;
}
...
/* ---------- TIMING BAR (serangan ala Undertale: makin tengah makin kuat) ---------- */
let timingBarState = null; // {pos, dir, running, onResult}
...
function stopTimingBar(){
  ...
  const dist = Math.abs(50 - timingBarState.pos); // 0 = tengah sempurna
  let power;
  if(dist<5) power = 1.8;       // sempurna
  else if(dist<15) power = 1.4; // bagus
  else if(dist<30) power = 1.0; // biasa
  else power = 0.6;             // meleset
...
function playerAttack(){
  ...
  startTimingBar((power)=>{
    const dmg = Math.max(1, Math.round(currentPlayerAtk()*power - battle.monster.def_ignore||0));
    battle.monster.hp -= dmg;
    battleLog(`Kamu menyerang! ${dmg} damage. (${power>=1.6?"SEMPURNA!":power>=1.2?"Bagus!":power>=0.9?"Kena":"Meleset"})`);
    afterPlayerAction();
  });
}
function playerUseSkill(skillId){
  ...
  document.getElementById("battle-actions").style.display="none";
  startTimingBar((power)=>{
    const dmg = Math.max(1, Math.round(currentPlayerAtk()*s.mult*power*0.6));
    battle.monster.hp -= dmg;
    battle.skillCooldowns[skillId] = s.cd;
    battleLog(`Kamu memakai ${s.name}! ${dmg} damage elemen ${s.element}.`);
    afterPlayerAction();
  });
}
...
function startDodgePhase(){
  ...
  dodgeGame = {
    player:{x:w/2, y:h-60, w:16, h:16},
    bullets:[],
    ticks: 0,
    maxTicks: 130,
    hit:false,
    speed: battle.monster.bulletSpeed * 2.6, // dikali karena kotak sekarang full layar (jauh lebih besar)
    spawnEvery: Math.max(6, 22 - battle.monster.floor*2),
...
function dodgeTick(){
  ...
  if(g.ticks % g.spawnEvery === 0 && g.ticks < g.maxTicks-20){
    const count = 1 + Math.floor(Math.random()*Math.min(3, battle.monster.bulletCount));
  ...
  // cek tabrakan
  const dodgeReduction = 1 - Math.min(0.7, (battle.playerBuffs.dodgeBoost + getPassiveDodgeBonus())/100);
  for(const b of g.bullets){
    const dx = b.x-g.player.x, dy=b.y-g.player.y;
    if(Math.sqrt(dx*dx+dy*dy) < 20){
      if(player.autoDodgeCharges>0 && player.autoDodgeCooldown<=0){
        player.autoDodgeCharges--;
        battleLog("✨ Auto Dodge mengaktifkan diri, menghindar otomatis!");
        b.y = h+999;
        continue;
      }
      if(Math.random() < dodgeReduction*0.001) continue; // sangat jarang skip
      g.hit = true;
      b.y = h+999;
    }
  }
...
function getPassiveDodgeBonus(){
  let bonus = 0;
  player.skills.forEach(id=>{
    if(id.includes("sk_dodge")){
...
function endDodgePhase(){
  ...
  if(dodgeGame.hit){
    if(player.autoDodgeCooldown<=0 && player.skills.includes(SKILLS.find(s=>s.name==="Auto Dodge").id) && player.autoDodgeCharges<=0){
      player.autoDodgeCharges = 3; player.autoDodgeCooldown = 3;
    }
    const dmg = Math.max(1, Math.round(battle.monster.atk - currentPlayerDef()*0.6));
    player.hp -= dmg;
    battleLog(`💥 Kamu terkena serangan ${battle.monster.name}! -${dmg} HP.`);
    onDodgeFail();
  } else {
    battleLog(`✅ Kamu berhasil menghindar semua serangan!`);
    const learned = onDodgeSuccessStreak();
    learned.forEach(s=> battleLog(`🎉 Skill baru didapat: ${s.name}!`));
    if(player.skills.includes(SKILLS.find(s=>s.name==="Auto Dodge").id) && player.autoDodgeCharges<=0 && player.autoDodgeCooldown<=0){
      player.autoDodgeCharges = 3;
    }
  }
  dodgeGame = null;
  renderBattle();
...
function onMonsterDefeated(){
  ...
  const floorId = battle.monster.floor;
  ...
  battle.ended = true;
  if(!wasBoss){
    player.floorProgress[floorId] = (player.floorProgress[floorId]||0)+1;
  } else {
    if(floorId===10){
      battleLog("🏆 SELAMAT! Kamu mengalahkan Penguasa Kegelapan dan menamatkan Kristal Saga!");
    } else {
      battleLog(`Pintu menuju lantai berikutnya kini terbuka!`);
      player.floorProgress[floorId+"_bossdone"] = true;
      player.maxFloorReached = Math.max(player.maxFloorReached, floorId+1);
    }
  }
  persist();
...
function endBattle(removeMonster){
  ...
  if(player.currentFloor===0){ showScreen("screen-overworld"); renderOverworld(); }
    ...
    if(removeMonster) removeCurrentMonsterSprite(monster);
    dungeonEncounterCooldown = Date.now() + 1200; // beri jeda supaya tidak langsung nabrak monster yg sama lagi
    renderDungeon();
  }
}
...
/* ============================== OVERWORLD (LOBBY GUILD) ============================== */
const lobbyBg = new Image();
let lobbyBgLoaded = false;
lobbyBg.onload = ()=>{ lobbyBgLoaded = true; };
lobbyBg.src = "assets/lobby_bg.png";

// Koordinat diukur ulang presisi per-area (crop+grid) dari gambar asli, skala k=0.5926 ke kanvas 910x607
const LOBBY_SOLIDS = [
  {x:130.4, y:68.1,  w:160.0, h:91.9},  // Toko Senjata & Armor
  {x:343.7, y:35.6,  w:160.0, h:124.4}, // Balai Guild
  {x:551.1, y:56.3,  w:142.4, h:115.6}, // Merchant Ramuan
  {x:728.9, y:65.2,  w:59.3,  h:112.6}, // Gerbang Dungeon - sisi kiri
  {x:841.5, y:65.2,  w:41.5,  h:112.6}, // Gerbang Dungeon - sisi kanan
  {x:23.7,  y:231.1, w:177.8, h:65.2},  // Penginapan
  {x:509.6, y:263.7, w:106.7, h:74.1},  // Bank
  {x:663.7, y:263.7, w:112.6, h:74.1},  // Tukang Besi
  {x:260.7, y:444.5, w:77.0,  h:47.4},  // Papan Misi
  {x:509.6, y:432.6, w:160.0, h:71.1},  // Pasar
  {x:35.6,  y:385.2, w:160.0, h:94.8},  // Rumah Penduduk (kiri, dekoratif)
  {x:714.1, y:382.2, w:133.3, h:97.8},  // Rumah Penduduk (kanan, dekoratif)
  {x:337.8, y:266.7, w:154.1, h:94.8},  // Air mancur tengah
  // Sungai, kiri & kanan jembatan
  {x:0,     y:530.4, w:343.7, h:76.5},
  {x:462.2, y:530.4, w:447.8, h:76.5},
];
function isLobbyBlocked(px,py){
  return LOBBY_SOLIDS.some(r=> px>r.x && px<r.x+r.w && py>r.y && py<r.y+r.h);
}
// Zona trigger: kotak SEMPIT tepat di depan pintu (bukan lingkaran besar) - hanya aktif kalau
// pemain berdiri persis di depan pintunya, bukan di samping/belakang bangunan.
const LOBBY_BUILDINGS = [
  {name:"Balai Guild",           action:"guild",           tx:343.7, ty:160.0, tw:160.0, th:45},
  {name:"Toko Senjata & Armor",  action:"shop",             tx:130.4, ty:160.0, tw:160.0, th:45},
  {name:"Merchant Ramuan",       action:"merchant",         tx:551.1, ty:171.9, tw:142.4, th:45},
  {name:"Gerbang Dungeon",       action:"dungeon_select",   tx:728.9, ty:177.8, tw:154.1, th:45},
  {name:"Penginapan",            action:"inn",              tx:23.7,  ty:296.3, tw:177.8, th:45},
  {name:"Bank",                  action:"bank",             tx:509.6, ty:337.8, tw:106.7, th:45},
  {name:"Tukang Besi",           action:"blacksmith",       tx:663.7, ty:337.8, tw:112.6, th:45},
  {name:"Papan Misi",            action:"questboard",       tx:260.7, ty:491.9, tw:77.0,  th:45},
  {name:"Pasar",                 action:"market",           tx:509.6, ty:503.7, tw:160.0, th:45},
];
let keysHeld = {};
...
function bindJoystick(prefix, targetKeys){
  ["left","right","up","down"].forEach(dir=>{
    ...
    if(!btn) return;
    const on = (e)=>{ if(e&&e.preventDefault) e.preventDefault(); targetKeys[dir]=true; };
    const off = ()=> targetKeys[dir]=false;
    // Pointer Events saja (gabungan touch+mouse) - hindari event ganda yg bikin arah "kedip-kedip"
...
function renderOverworld(){
  ...
  function loop(){
    ...
    const spd = 3.6;
    let dx=0, dy=0;
    if(keysHeld.left) dx -= spd;
    if(keysHeld.right) dx += spd;
    if(keysHeld.up) dy -= spd;
    if(keysHeld.down) dy += spd;
    const nx = Math.max(24, Math.min(886, player.pos.x+dx));
    const ny = Math.max(24, Math.min(583, player.pos.y+dy));
    if(!isLobbyBlocked(nx, player.pos.y)) player.pos.x = nx;
    if(!isLobbyBlocked(player.pos.x, ny)) player.pos.y = ny;
...
function drawOverworld(canvas){
  ...
  // NPC pengembara di depan tenda Merchant
  drawMonsterSprite(ctx, "wierd_traveler", 705, 190, 32, true);
  drawPlayerSprite(ctx, player.pos.x, player.pos.y, player.facing, player.isMoving);

  LOBBY_BUILDINGS.forEach(b=>{
    if(player.pos.x>b.tx && player.pos.x<b.tx+b.tw && player.pos.y>b.ty && player.pos.y<b.ty+b.th){
...
function goToLobby(){
  player.currentFloor = 0;
  player.pos = {x:450,y:250};
  showScreen("screen-overworld");
  renderOverworld();
  persist();
}

/* ============================== DUNGEON (2D per lantai) ============================== */
let dungeonMonsters = [];
let dungeonEncounterCooldown = 0; // jeda sesaat setelah keluar battle biar gak langsung nabrak lagi (fix "kabur tapi tetap fight")
function enterDungeon(floorId){
  player.currentFloor = floorId;
  player.pos = {x:60,y:300};
  const d = DUNGEONS.find(x=>x.id===floorId);
  dungeonMonsters = [];
  if(d.finalBoss){
    dungeonMonsters.push({ x:600, y:200, alive:true, isBoss:true, uid:"finalboss", spriteKey: pickMonsterSprite(floorId,true) });
  } else {
    const killed = player.floorProgress[floorId]||0;
    const remain = Math.max(0, d.monsterCount - killed);
    for(let i=0;i<Math.min(remain,8);i++){
      dungeonMonsters.push({ x:150+Math.random()*450, y:80+Math.random()*300, alive:true, isBoss:false, uid:"m"+i, spriteKey: pickMonsterSprite(floorId,false) });
    }
    if(remain<=0){
      dungeonMonsters.push({ x:640, y:220, alive:true, isBoss:true, uid:"boss", spriteKey: pickMonsterSprite(floorId,true) });
    }
  }
  showScreen("screen-dungeon");
  renderDungeon();
...
function removeCurrentMonsterSprite(monster){
  // Cocokkan berdasarkan uid sprite yang MEMICU battle ini (bukan asal ambil monster hidup pertama)
  let idx = -1;
  if(monster.sourceUid){
    idx = dungeonMonsters.findIndex(m=>m.alive && m.uid===monster.sourceUid);
  }
  if(idx<0){ // fallback lama, kalau2 sourceUid tak tersedia
    idx = dungeonMonsters.findIndex(m=>m.alive && (m.isBoss===monster.isBoss));
  }
...
function renderDungeon(){
  ...
  document.getElementById("dungeon-title").textContent = `Lantai ${d.id}: ${d.name}`;
...
function drawDungeon(canvas, d){
  ...
  ctx.fillText("🚪", 25, 300);
  ctx.font="11px sans-serif"; ctx.fillStyle="#cbb"; ctx.fillText(d.id===1?"Ke Lobby":"Ke Lantai "+(d.id-1), 25, 320);
  ...
  dungeonMonsters.filter(m=>m.alive).forEach(m=>{
    ...
    if(Date.now() > dungeonEncounterCooldown && Math.sqrt(dx*dx+dy*dy) < 30){
      const monster = makeMonster(player.currentFloor, m.isBoss, m.spriteKey, m.uid);
      startBattle(monster);
    }
  });
}
...
function exitDungeonOneLevel(){
  ...
  if(player.currentFloor<=1){ goToLobby(); }
  else { enterDungeon(player.currentFloor-1); }
}
function openDungeonSelect(){
  const list = document.getElementById("dungeon-select-list");
  list.innerHTML = "";
  DUNGEONS.forEach(d=>{
    const unlocked = d.id===1 || player.maxFloorReached>=d.id || (player.floorProgress[(d.id-1)+"_bossdone"]);
    const div = document.createElement("div");
    div.className = "list-item"+(unlocked?"":" locked");
    div.innerHTML = `<b>Lantai ${d.id}: ${d.name}</b><br><small>${d.finalBoss? "Bos Utama: "+d.boss : (d.monsterCount+" monster + Bos: "+d.boss)}</small>`;
    if(unlocked) div.onclick = ()=>{ closeAllModals(); enterDungeon(d.id); };
...
function renderShop(){
  ...
  items.filter(i=>!i.starter).forEach(item=>{
    ...
    const div = document.createElement("div");
    div.className = "list-item";
    div.innerHTML = `<span>${item.icon} ${item.name} <small>(${stat})</small><br><small>Harga: ${priceLabel}</small></span>
...
function renderInventory(){
  ...
  player.inventory.weapons.forEach(id=>{
    ...
    html += `<div class="list-item"><span>${w.icon} ${w.name} (ATK+${w.atk}${lvl>0?`, +${lvl*4} upgrade`:""})</span><button ${eq?"disabled":""} onclick="equipItem('weapon','${id}')">${eq?"Terpakai":"Pakai"}</button></div>`;
  });
  html += `<h3>🛡️ Armor</h3>`;
  ...
  player.inventory.armors.forEach(id=>{
    ...
    const eq = player.equipped.armor===id;
    const lvl = getUpgradeLevel(id);
    html += `<div class="list-item"><span>${a.icon} ${a.name} (DEF+${a.def}${lvl>0?`, +${lvl*3} upgrade`:""})</span><button ${eq?"disabled":""} onclick="equipItem('armor','${id}')">${eq?"Terpakai":"Pakai"}</button></div>`;
  });
  html += `<h3>💫 Amulet</h3>`;
  ...
  player.inventory.amulets.forEach(id=>{
    ...
    const eq = player.equipped.amulet===id;
    html += `<div class="list-item"><span>${a.icon} ${a.name} (ATK+${a.atk}/DEF+${a.def})</span><button ${eq?"disabled":""} onclick="equipItem('amulet','${id}')">${eq?"Terpakai":"Pakai"}</button></div>`;
  });
  html += `<h3>📜 Skill Dipelajari (${player.skills.length})</h3>`;
  if(player.skills.length===0) html += `<small>Belum ada skill. Bunuh monster & hindari serangan untuk belajar skill.</small>`;
  player.skills.forEach(sid=>{
    const s = getSkill(sid);
...
function openSkillMenu(){
  ...
  const atkSkills = player.skills.map(getSkill).filter(s=>s.type==="attack");
...
function goFullscreen(){
  ...
  const doLock = ()=>{
    if(screen.orientation && screen.orientation.lock){
      screen.orientation.lock("landscape").catch(()=>{ /* tidak didukung browser ini, cukup mode fullscreen saja */ });
    }
  };
  if(req){
...
function initGame(){
  initKeyboard();
  bindJoystick("owjoy", keysHeld);   // joystick di layar lobby
  bindJoystick("dgjoy", keysHeld);   // joystick di layar dungeon (id beda, target keysHeld sama)
